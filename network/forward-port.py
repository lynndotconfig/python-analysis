...
class Receiver(asyncore.dispatcher):
    ...
    def handle_read(self):
        ...
        print "Receive Read: %s" % read
        self.from_remote_buffer += read

    def writable(self):
        print "Receive writable len: %s" % len(self.to_remote_buffer)
        return (len(self.to_remote_buffer) > 0)

    def handle_write(self):
        sent = self.send(self.to_remote_buffer)
        print "Receive Sent: %s" % sent
...
class Sender(asyncore.dispatcher):
    ...
    def __init__(self, receiver, remoteaddr, remoteport):
        print "Init sender with:%s " % (receiver)
        asyncore.dispatcher.__init__(self)
    ...
    def handle_read(self):
        read = self.recv(BUFSIZE)
        print "Sender Read: %s" % read
        self.receiver.to_remote_buffer += read

    def writable(self):
        print "Sender writable len:%d" % len(self.receiver.from_remote_buffer)
        return (len(self.receiver.from_remote_buffer) > 0)

    def handle_write(self):
        print "Sender write: %s" % self.receiver.from_remote_buffer
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Port Forward Example with Asyncore.")
    ...
    parser.add_argument('---remote-port', action='store', dest='remote_port', type=int, default=8080)
